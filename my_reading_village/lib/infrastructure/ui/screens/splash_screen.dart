...
import 'package:my_reading_village/infrastructure/ui/screens/game_screen.dart';
...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Future<void> _scheduleStoreDiscountNotifications(
    ...
    final notifications =
        <({DateTime scheduledAt, String title, String body})>[];
    ...
    await notif.scheduleStoreDiscountNotifications(
  ...
  Future<void> _scheduleEventReminders(
    ...
    for (int i = 0; i < HolidayRules.allEvents.length; i++) {
      ...
      final startNotif =
          start.isAfter(now) ? start : now.add(const Duration(hours: 1));
      notifs.add((
        scheduledAt: startNotif,
        title: lang
            .translate('notif_event_start_title')
            .replaceAll('{event}', eventName),
        body: lang
            .translate('notif_event_start_body')
            .replaceAll('{event}', eventName),
      ));
      ...
      if (mid.isAfter(now)) {
        notifs.add((
          scheduledAt: mid,
          title: lang
              .translate('notif_event_mid_title')
              .replaceAll('{event}', eventName),
          body: lang
              .translate('notif_event_mid_body')
              .replaceAll('{event}', eventName),
        ));
      }
      ...
      if (endingWarning.isAfter(now)) {
        notifs.add((
          ...
          title: lang
              .translate('notif_event_ending_title')
              .replaceAll('{event}', eventName),
          body: lang
              .translate('notif_event_ending_body')
              .replaceAll('{event}', eventName),
        ));
      }
