...
class _StatsContent extends StatelessWidget {
  ...
  const _StatsContent(
      {required this.village, required this.bookProvider});
...
class _ChartsContentState extends State<_ChartsContent> {
            ...
            child: Row(
              ...
              children: [
                Icon(Icons.auto_stories,
                    size: 12, color: AppTheme.lavender),
  ...
  Widget _buildChart(
      BuildContext context, Map<int, int> data, String locale) {
    final pagesLabel = context.read<LanguageProvider>().translate('chart_pages').toLowerCase();
    final labels = _buildLabels(context, locale);
    final count = labels.length;

    final maxVal = data.values.isEmpty
        ? 0
        : data.values.reduce(max);
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color:
                selected ? AppTheme.darkLavender : AppTheme.lavender,
          ),
        ),
        child: Text(
          label,
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Text(label,
              style:
                  TextStyle(fontSize: 15, color: AppTheme.darkText)),
          const Spacer(),
          Text(value,
              style: TextStyle(
                  fontSize: 18,
